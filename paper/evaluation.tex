...
\tkcomment{A different architecture such as ARM + NEON, or ideally ARM + SVE
would be nice.}
...
hull. Our implementation disagrees on $20$ points. These are points $u$ for which
...
The implementations agree on input sizes where all points are found. If we
create a circle by computing 

$$C_n := \left\{\left(\cos\left(\frac{2i\pi}{2^n}\right), \sin\left(\frac{2i\pi}{2^n}\right)\right) \ | \ 0 \leq i < 2^n\right\}$$

and randomly permuting the points, we do get the same points back for at least
$2^{27} \approx 1.3 \cdot 10^8$ points.
